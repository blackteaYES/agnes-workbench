...
- **图像生成** — 文生图 / 图生图 / 多图合成，风格预设、提示词工作台（随机灵感 / 优化提示词）、输出档位与宽高比可调，参考图可从本地或作品库选择（长按拖拽排序尚未完成）。
- **视频生成** — 文生视频 / 图生视频 / 关键帧动画，异步任务 + 智能轮询（含限流退避），可手动刷新状态。
- **作品历史** — 仅保存媒体 URL、提示词摘要与元数据到浏览器 localStorage，绝不保存 API 密钥；媒体预览支持左右切换。
